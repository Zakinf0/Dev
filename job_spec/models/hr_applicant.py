...
class Applicant(models.Model):
    ...
    tell_me_about_your_self = fields.Text(string="1. Tell me about yourself", required = True)
    when_can_you_start = fields.Text(string="2. When can you start", required = True)
    what_are_your_salary_expectations = fields.Text(string="3. After you have went through the job spec for this role, what are your salary expectations?", required = True)
    why_are_you_in_the_job_market_at_the_moment = fields.Text(string="4. Why are you in the job market at the moment", required = True)
    how_do_you_feel_about_counter_offer = fields.Text(string="5. How do you feel about counter offer", required = True)
    what_are_possibilities_of_counter_offer = fields.Text(string="6. If my client offers you this job,"
                                                             "what are the possibility of your current company to counter offer?", required = True)
    what_are_your_proudest_professional_achievements = fields.Text(string="7. What are your proudest professional achievements?", required = True)
    what_is_your_idea_work_environment = fields.Text(string="8. What's your ideal work environment?", required = True)
    #zakinfo developmemt team
    are_you_interviewing_with_other_companies = fields.Text(string="9. Are you interviewing with other companies", required = True)
    when_would_you_be_available_to_start_a_new_role = fields.Text(string="10. When would you be available to start a new role", required = True)

    # Experience and backgrund tab
    knowledge_areas = fields.Char(string='In what areas are you most knowledgeable?', required = True)
    strengths = fields.Char(string='What are your strengths?', required = True)
    best_work_environment = fields.Char(string='What kind of environment do you need to do your best work?', required = True)
    how_do_you_work_under_pressure = fields.Char(string='How do you work under pressure?', required = True)
    leadership_style = fields.Char(string='What\'s your leadership style?', required = True)
    leadership_example = fields.Char(string='Tell me about when you used leadership skills to get a job done.', required = True)
    typical_day = fields.Char(string='Describe a typical day at your current job.', required = True)
    career_accomplishments = fields.Char(
        string='Tell me about one of your most significant career accomplishments so far.', required = True)
    conflict_handling_with_colleague = fields.Char(
        string='Was there a time you didn\'t work well with a manager or colleague? If so, can you tell me how you handled the situation?', required = True)
    sample_work = fields.Char(string='Can you provide us with a sample of your work?', required = True)
    motivation = fields.Char(string='What motivates you?', required = True)
    next_job_expectations = fields.Char(string='What are you looking for in your next job?', required = True)
    other_expectations_with_new_job = fields.Char(string='Others you looking for in your next job?', required = True)
    others = fields.Char(string='Others', required = True)
